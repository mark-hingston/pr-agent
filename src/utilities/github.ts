 ...
 */
export const getIssueComments = async () => { // Added return type Promise<any[]> for clarity, though Octokit types might be better
    try {
        const octokit = getOctokitClient();
        ...
        const { data: comments } = await octokit.rest.issues.listComments({
            ...
            // per_page: 100, // Consider adding pagination if many comments are expected
        });
...
/** Cached username of the authenticated bot user. */
let botUsername: string | null = null;
/**
 * Retrieves the username of the authenticated bot user (GitHub Action).
 * Caches the username after the first successful retrieval.
 * @throws {Error} If fetching the authenticated user fails.
 * @returns A Promise resolving to the bot's username string.
 */
export const getBotUsername = async (): Promise<string> => {
    ...
    try {
        ...
        console.log(`Authenticated as bot: ${botUsername}`);
        return botUsername;
    } catch (error) {
        console.error("Error getting authenticated user login:", error);
